...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (carouselContainer && LOVE_DATA.gallery && LOVE_DATA.gallery.length > 0) {
    ...
    const DRAG_SENSITIVITY = 0.04;    // degrees per pixel dragged
    ...
    // ---- Positioning ----
    function positionSlides(rotation) {
      const slides = ring.querySelectorAll(".carousel-slide");
      const total = slides.length;
      const angleStep = 360 / total;
      const containerWidth = carouselContainer.clientWidth;
      const radius = Math.max(250, Math.min(500, (containerWidth || 800) * 0.55));

      slides.forEach(function(slide, i) {
        const angle = angleStep * i + (rotation || 0);
        const rad = (angle * Math.PI) / 180;
        const x = radius * Math.sin(rad);
        const z = radius * Math.cos(rad);
        // No rotateY — every slide always faces the camera (billboard-style)
    ...
    function updateVisibleSlides(currentRotY) {
      ...
      slides.forEach(function(slide, i) {
        ...
        const opacity = Math.max(0.08, 1 - (dist / 170) * 0.7);
    ...
    function animate() {
      ...
      // Apply baseline auto-rotation (paused while dragging)
      if (!isDragging) {
        rotY += BASE_SPEED;
      }
      ...
      // Reposition slides on the circle (always facing camera)
      positionSlides(rotY);
    ...
    function onDragStart(clientX) {
      isDragging = true;
      prevX = clientX;
      extraVelocity = 0;  // reset residual momentum so drag starts clean
